local Utils = require("barthap.utils")
...
-- lazy
map("n", "<leader>l", "<cmd>:Lazy<cr>", { desc = "Lazy.nvim config" })
...
map("n", "<leader>wh", "<C-W>v", { desc = "Split window right" })

-- Prev/Next code location
--
map("n", "<C-[", "<C-o>", { desc = "Prev location" })
map("n", "<C-]", "<C-i>", { desc = "Next location" })

-- Prev/Next word occurrence under cursor
-- In addition to [[ and ]] added by illuminate plugin
...
-- toggle options
map("n", "<leader>uf", require("barthap.plugins.lsp.format").toggle, { desc = "Toggle format on Save" })
...
-- floating terminal
map("n", "<leader>fT", function() Utils.float_term(nil, { cwd = Utils.get_root() }) end, { desc = "Terminal (detect root)" })
